...
function injectStyles(): void {
  ...
  style.id = STYLE_ID;
  style.textContent = `
    #${BUTTON_ID}{position:fixed;left:10px;bottom:10px;z-index:99988;width:32px;height:32px;padding:0;display:grid;place-items:center;border:1px solid #1e3a3a;border-radius:8px;background:rgba(0,0,0,.75);color:#a4f5f5;font-size:16px;cursor:pointer;box-shadow:0 8px 24px rgba(0,0,0,.45)}
    #${PANEL_ID}{position:fixed;inset:0;z-index:999994;display:grid;place-items:center;padding:18px;box-sizing:border-box;background:transparent;pointer-events:none;color:#e0e0e0;font:11px/1.4 ui-monospace,monospace}
    #${PANEL_ID}[hidden]{display:none}
    #${PANEL_ID} .go-stage{display:flex;align-items:flex-start;gap:8px;pointer-events:none}
    #${PANEL_ID} .go-card{width:min(344px,94vw);max-height:90vh;display:flex;flex-direction:column;overflow:hidden;pointer-events:auto;border:1px solid #1e3a3a;border-radius:12px;background:#0d1117;box-shadow:0 8px 32px rgba(0,0,0,.8)}
    #${PANEL_ID} .go-config-card{width:300px;max-height:90vh;display:flex;flex-direction:column;overflow:hidden;pointer-events:auto;border:1px solid #1e3a3a;border-radius:8px;background:#0a1f1f;box-shadow:0 4px 20px rgba(0,0,0,.6)}
    #${PANEL_ID} header{display:flex;align-items:center;justify-content:space-between;padding:12px 16px;color:#a4f5f5;background:#0a1f1f;border-bottom:1px solid #1a2a2a;cursor:move}
    #${PANEL_ID} h2{flex:0 0 auto;margin:0;white-space:nowrap;font:700 12px/1.2 ui-monospace,monospace;letter-spacing:.02em}
    #${PANEL_ID} header .go-actions{display:flex;flex:0 0 auto;align-items:center;gap:4px}
    #${PANEL_ID} header button,#${PANEL_ID} button{padding:5px 7px;border:1px solid #1e3a3a;background:#0c2020;color:#8fc4c4;border-radius:7px;cursor:pointer;font:600 9px ui-monospace,monospace}
    #${PANEL_ID} header button{width:24px;min-width:24px;height:24px;padding:0;border:0;border-radius:4px;background:rgba(255,255,255,.08);color:#7ab8b8;font-size:12px}
    #${PANEL_ID} header button[data-zoom]{width:32px;font-size:8px}
    #${PANEL_ID} header button[data-close]{color:#fff;background:#c0392b}
    #${PANEL_ID} header button:hover,#${PANEL_ID} button:hover{color:#d6f7f7;background:#123333}
    #${PANEL_ID} header button[data-active=true]{color:#a4f5f5;background:rgba(164,245,245,.2)}
    #${PANEL_ID} .go-body{min-height:0;overflow:auto;padding:0;background:#0d1117;scrollbar-width:thin;scrollbar-color:#1e3a3a transparent}
    #${PANEL_ID} .go-config-body{min-height:0;overflow:auto;background:#0d1117}
    #${PANEL_ID} .go-section{padding:10px 16px;border-bottom:1px solid #1a2a2a}#${PANEL_ID} .go-growth,#${PANEL_ID} .go-estimates{padding:12px 16px}
    #${PANEL_ID} .go-section-title{display:flex;align-items:center;justify-content:space-between;margin-bottom:5px;color:#4a8a8a;font-size:10px;font-weight:700;letter-spacing:.1em;text-transform:uppercase}
    #${PANEL_ID} .go-section-title::after{content:'';height:1px;flex:1;margin-left:7px;background:#1e3a3a}
    #${PANEL_ID} .go-section-title>span:last-child{order:2;margin-left:7px;white-space:nowrap;font-size:8px;font-weight:400;letter-spacing:0}
    #${PANEL_ID} .go-summary{display:grid;grid-template-columns:repeat(2,1fr);gap:5px;margin:0}
    #${PANEL_ID} .go-metric{padding:6px 8px;border:1px solid #0f2020;border-radius:6px;background:#081818}
    #${PANEL_ID} .go-metric small{display:block;color:#4a8a8a;font-size:8px;text-transform:uppercase;letter-spacing:.08em}#${PANEL_ID} .go-metric b{color:#4caf50;font:700 13px/1.3 ui-monospace,monospace}
    #${PANEL_ID} .go-metric.go-growing b{color:#e6b800;font-size:16px}#${PANEL_ID} .go-metric.go-size b{color:#ff8c00;font-size:16px}
    #${PANEL_ID} .go-progress{padding:5px 0}
    #${PANEL_ID} .go-progress>div{display:flex;align-items:center;justify-content:space-between;margin-bottom:4px;color:#7ab8b8;font-size:12px}
    ...
    #${PANEL_ID} .go-progress b{font:700 11px ui-monospace,monospace}#${PANEL_ID} .go-progress>i{display:block;height:4px;overflow:hidden;border-radius:2px;background:#1a2e2e}
    #${PANEL_ID} .go-progress>i u{display:block;height:100%;border-radius:3px;text-decoration:none}
    #${PANEL_ID} .go-section-head{display:flex;align-items:center;justify-content:space-between;gap:8px;margin-bottom:4px}#${PANEL_ID} .go-section-head .go-section-title{flex:1;margin:0}
    #${PANEL_ID} .go-section-head button{width:26px;height:26px;padding:0;font-size:13px}
    #${PANEL_ID} .go-eta-detail{padding:8px 0;color:#7ab8b8}#${PANEL_ID} .go-eta-detail>div,#${PANEL_ID} .go-eta-done{display:flex;align-items:center;justify-content:space-between}#${PANEL_ID} .go-eta-detail span,#${PANEL_ID} .go-eta-done span{display:flex;align-items:center;gap:7px;font-size:12px}#${PANEL_ID} .go-eta-detail span i,#${PANEL_ID} .go-eta-done span i{width:6px;height:6px;flex:0 0 auto;border-radius:50%}#${PANEL_ID} .go-eta-detail b{font-size:12px}#${PANEL_ID} .go-eta-detail em{color:#555;font-size:10px;font-weight:400;font-style:normal}#${PANEL_ID} .go-eta-detail>u{display:block;height:5px;margin-top:5px;overflow:hidden;border-radius:3px;background:#1a2e2e;text-decoration:none}#${PANEL_ID} .go-eta-detail>u i{display:block;height:100%;border-radius:3px}#${PANEL_ID} .go-eta-detail>small{display:block;margin-top:4px;color:#3a6a6a;text-align:right;font-size:10px}#${PANEL_ID} .go-eta-done{padding:6px 0;color:#7ab8b8}#${PANEL_ID} .go-eta-done span i{background:#34d399}#${PANEL_ID} .go-eta-done b{color:#34d399;font-size:11px}
    #${PANEL_ID} .go-plants{padding-left:10px}#${PANEL_ID} .go-plant-row{display:flex;justify-content:space-between;padding:2px 0;color:#7ab8b8;font-size:12px}
    #${PANEL_ID} .go-plant-row b{font-weight:700}
    #${PANEL_ID} .go-footer{display:flex;align-items:center;justify-content:space-between;padding:8px 16px;background:#0a1f1f;border-top:1px solid #1e3a3a;color:#4a8a8a}
    #${PANEL_ID} .go-footer span{display:flex;align-items:center;gap:8px}#${PANEL_ID} .go-footer span small{padding:1px 6px;border-radius:3px;background:#1e3a3a;color:#4a8a8a;font-size:10px}#${PANEL_ID} .go-footer b{color:#ffd84d;font-size:20px}
    #${PANEL_ID} .go-filter{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:5px;max-height:250px;margin:8px 0 12px;overflow:auto}#${PANEL_ID} .go-filter label{display:flex;align-items:center;gap:6px;padding:6px;border:1px solid #1e3a3a;border-radius:7px;background:#0c2020;color:#8fc4c4}
    #${PANEL_ID} .go-tools{display:flex;align-items:center;justify-content:space-between;gap:6px;margin:0 0 8px}#${PANEL_ID} .go-search{width:100%;box-sizing:border-box;margin-bottom:8px;padding:7px 9px;border:1px solid #244944;border-radius:6px;outline:none;background:#051311;color:#d9f0ec;font:10px system-ui,sans-serif}
    #${PANEL_ID} .go-pill-list{max-height:320px;overflow:auto}#${PANEL_ID} .go-pill-section{margin:9px 0}#${PANEL_ID} .go-pill-section>b{display:flex;align-items:center;justify-content:space-between;margin-bottom:5px;color:#4a8a8a;font-size:9px;letter-spacing:.08em;text-transform:uppercase}#${PANEL_ID} .go-pill-section>b button{padding:3px 7px;font-size:9px;text-transform:none}#${PANEL_ID} .go-pill-section>div{display:flex;flex-wrap:wrap;gap:4px}
    #${PANEL_ID} button.go-pill{display:inline-flex;align-items:center;gap:5px;padding:4px 9px;border:1px solid #1e3a3a;border-radius:8px;background:#0c2020;color:#8fc4c4;font-size:10px;white-space:nowrap}#${PANEL_ID} button.go-pill.on{border-color:#3aa6a0;background:linear-gradient(180deg,#11524d,#0c3d3a);color:#eaffff;box-shadow:0 0 0 1px rgba(58,166,160,.25)}#${PANEL_ID} button.go-pill i{color:#41e0c8;font-size:9px;font-style:normal}#${PANEL_ID} button.go-pill small{opacity:.5;font-size:9px}
    #${PANEL_ID} .go-search:focus{border-color:#4fa99e}#${PANEL_ID} .go-collapsible{cursor:pointer;margin:0}#${PANEL_ID} .go-muted{color:#527d78;font-size:9px}
    #${PANEL_ID} .go-config-row{display:flex;align-items:center;justify-content:space-between;gap:10px;padding:6px 0;border-top:1px solid #102825}
    #${PANEL_ID} .go-config-row select{max-width:150px;padding:5px;border:1px solid #1e3a3a;border-radius:5px;background:#0d1117;color:#a4d5d5;font:10px ui-monospace,monospace}
