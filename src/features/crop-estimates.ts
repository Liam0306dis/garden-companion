...
function relayoutNativeEstimates(view: PixiNode, signature: string): void {
  ...
  // No position test on top: a row that fills the column can start left of `columnLeft`, which is only
  // an estimate of the game's column edge, and dropping such a row stacked our line over it.
  const isGameRow = (child: PixiNode) => GAME_ROW_LABELS.includes(child.label) || (!child.label && holdsGameLabel(child));
